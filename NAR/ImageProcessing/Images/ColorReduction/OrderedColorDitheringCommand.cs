...
{
    public class OrderedColorDitheringCommand : IColorReduction, ICommand
    {
        #region Variables
        private int _rows = 4;
        private int _cols = 4;
        ...
        public OrderedColorDitheringCommand()
        {
        }
        public OrderedColorDitheringCommand(byte[,] matrix)
        {
        ...
        public Model.IImage Execute(Model.IImage image)
        {
            ...
            for (int line = 0; line < height; line++)
            {
                ...
                {
                    byte value = (byte)((result[pos] <= _matrix[(line % _rows), (col % _cols)]) ? 0 : 255);
